...
frontEndDeploymentConfigs.items.each {Map object ->
    ...
    OpenShiftHelper._exec(["bash", '-c', "oc process -f openshift/templates/sonar.pod.json -l 'app=dsrp-${appLabel},sonar=${config.app.build.id}-${object.metadata.name}' -p 'NAME=sonar-${config.app.build.id}-${object.metadata.name}' -p 'IMAGE=${isTag.image.dockerImageReference}' -p 'DB_CONFIG_NAME=${dbConfig}' -p 'GIT_BRANCH=${branch}' -p 'CPU_LIMIT=1500m' --namespace=${object.metadata.namespace} |  oc replace -f - --namespace=${object.metadata.namespace} --force=true"], new StringBuffer(), new StringBuffer())
}

// Run backend tests
backEndDeploymentConfigs.items.each {Map object ->
    Map isTag = ocGet(["istag/${object.metadata.name}:${appLabel}", "--namespace=${namespace}"])
    OpenShiftHelper._exec(["bash", '-c', "oc process -f openshift/templates/sonar.pod.json -l 'app=dsrp-${appLabel},sonar=${config.app.build.id}-${object.metadata.name}' -p 'NAME=sonar-${config.app.build.id}-${object.metadata.name}' -p 'IMAGE=${isTag.image.dockerImageReference}' -p 'DB_CONFIG_NAME=${dbConfig}' -p 'DOCUMENT_GENERATOR_URL=${documentGeneratorURL}' -p 'GIT_BRANCH=${branch}' --namespace=${object.metadata.namespace} |  oc replace -f - --namespace=${object.metadata.namespace} --force=true"], new StringBuffer(), new StringBuffer())
}

if (!OpenShiftHelper.waitForPodsToComplete(['pods','-l', "app=dsrp-${appLabel},sonar", "--namespace=${namespace}"])){
    System.exit(1)
}
